...
# 设置日志格式和日志级别，并将日志输出到logs/my_rewinding_loader_{timestamp}.log文件中
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s',
    filename=f'logs/my_rewinding_loader_{timestamp}.log',
    filemode='w'
)


def main():
    # 按行读取当前目录下的my_rewinding_loader_input.txt文件
    with open('my_rewinding_loader_input.txt', 'r') as f:
        dirs = f.readlines()
        # 去除每行末尾的换行符
    ...
    for from_dir_val in dirs:
        command = f"{python_executable} experiments/rewinding.py {batch_size_opt} {batch_size_val} {from_dir_opt} {from_dir_val}"
